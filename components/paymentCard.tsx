...
export default function PaymentCard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const normalizedItems: CartItem[] = Array.isArray(order.items)
      ...
      setShippingFee(Number.isFinite(fee) && fee > 0 ? fee : 15);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const alreadyExists = orders.some((o: any) => o.orderId === orderId);
    ...
    if (!alreadyExists) {
      ...
      localStorage.setItem(`paymentExpireAt_${orderId}`, String(expireAt));
    }
  }, [orderId, items, total]);
      ...
      if (diff <= 0) {
        ...
        if (existingOrders) {
          ...
          const updatedOrders = orders.map((order: any) => {
            if (order.orderId === orderId && order.status === "toPay") {
              return {
                ...order,
                status: "cancelled",
              };
  ...
  const merchantName = useMemo(() => {
    if (!items.length) return "BazaarFlow";

    return Array.from(new Set(items.map((i) => i.title))).join(", ");
  }, [items]);
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border bg-white shadow-sm">
      <div className="bg-[#d93f1d] px-6 py-5 text-center text-white">
        <h1 className="text-2xl font-bold">Thai QR Payment</h1>

        <div className="mt-3 text-sm">
          Remaining: <b>{expireTime}</b>
        </div>
      </div>
          ...
          <button
            onClick={() => {
              ...
              if (storedOrder) {
                ...
                localStorage.setItem("paymentTotal", JSON.stringify(total));
              ...
              router.push(`/${mockUser.id}/payment/success`);
            }}
